...
```{r}
income_by_state <- gather(income_by_state, key = year, value = value,
                          -GeoName, - Description)
...
income_by_state2$state <- gsub(pattern = "*", replacement = "",
                               x = income_by_state2$state, fixed = T)
```
